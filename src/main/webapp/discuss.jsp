...
<span class="layui-breadcrumb" lay-separator="—">
  <a href="">首页</a>
  <a href="">菜品模块</a>
  <a><cite>菜品评论管理</cite></a>
</span>
<div>
    <label>&nbsp;</label>
</div>

<form action="${pageContext.request.contextPath}/queryDiscussByPage2"  method="post">
...
<table class="layui-table layui-form">
    ...
    <c:forEach items="${pageinfo.list }" var="str">
        <tr>
            <td>${str.discussId }</td>
            <td>${str.discussContent }</td>
            <td>${str.shopId}</td>
            <td>${str.shopName}</td>
            <td>用户名</td>
            <td>${str.dishesId}</td>
            <td>${str.dishesName}</td>
            <td>${str.discussTime}</td>
            <td>
                   <a class="layui-btn layui-btn-sm <!-- layui-btn-danger -->" title="删除" onclick="javascript:return confirm('确定移除该菜品？');" href="${pageContext.request.contextPath}/deleteDiscuss?discussId=${str.discussId }">
